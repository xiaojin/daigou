...
#pragma mark - 显示/隐藏
- (BOOL)isSidebarShown{
    return self.contentView.frame.origin.x > -sideBarWith ? YES :NO;
}

- (void)showHideSidebar
{
    if (self.contentView.frame.origin.x == -sideBarWith) {
        LLLog(@"开始时在-点");
...
- (void)autoShowHideSidebar
{
    ...
    else
    {
        LLLog(@"自动缩回");
        [UIView animateWithDuration:duration animations:^{
            [self setSidebarOriginX:-sideBarWith];
        } completion:^(BOOL finished) {
            _isMoving = NO;
            self.view.hidden = YES;
        }];
...
/*
 * 设置侧栏位置
 * 完全不显示时为x=-sideBarWith，显示到最右时x=0
 */
...
- (void)setSidebarOffset:(CGFloat)offset
{
    ...
    if (offset >=0) { // 右滑
        ...
        // 如果不在最左
        if (rect.origin.x > -sideBarWith) {
            rect.origin.x = startContentOriginX + offset;
            if (rect.origin.x < -sideBarWith) {
                rect.origin.x = -sideBarWith;
            }
        }
    }
    [self.contentView setFrame:rect];
...
{
    CGRect rect = self.contentView.frame;
    float percent = (sideBarWith + rect.origin.x) / sideBarWith;
